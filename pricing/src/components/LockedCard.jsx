export default function LockedCard({ plan, isUnlocked, onSubscribe }) {
  if (isUnlocked) {
    return (
      <div className="glass rounded-glass p-8 text-center flex flex-col">
        ...
        <ul className="text-left space-y-3 flex-1 mb-6 text-sm text-sage">
          {plan.features.map((f, i) => (
            <li key={i} className="flex items-center gap-2">
              <span className="w-4 h-4 rounded-full bg-mint/70 flex-shrink-0" />
              {f}
            </li>
          ))}
        </ul>
        ...
        <button
          ...
          className="btn-ink w-full text-[0.95rem] py-3"
        >
          Get {plan.label}
        </button>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="rounded-full bg-stone/40 text-sage py-3 px-6 text-sm flex items-center justify-center gap-2">
        ...
        Unlocks once founding spots fill
      </div>
    </div>
  );
}

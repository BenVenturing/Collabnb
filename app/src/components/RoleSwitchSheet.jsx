...
export default function RoleSwitchSheet({ open, targetRole = 'host', onClose }) {
  ...
  const roleLabel = isHostSignup ? 'Host' : 'Creator';
  ...
  async function handleSubmit(e) {
    ...
    if (!isHostSignup && !instagram.trim()) {
      setError('An Instagram handle is required so we can review your profile.');
      return;
    }
    setSubmitting(true);
    setError('');
        ...
        {!done ? (
          <>
            ...
            <p style={{ color: 'var(--slate)', fontSize: '0.875rem', lineHeight: 1.6, marginBottom: '1.25rem' }}>
              {isHostSignup
                ? "You'll join as a host using this same account. Like every new host, your details go through a quick review before host tools unlock — you'll keep full creator access in the meantime."
                : "You'll join as a creator using this same account. Like every new creator, your profile goes through a quick review before creator access unlocks — you'll keep full host access in the meantime."}
            </p>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {isHostSignup ? (
                <>
                  <div className="form-group">
                    <label className="form-label" htmlFor="rs-business">Property or business name</label>
                    <input id="rs-business" className="form-input" type="text" value={business} onChange={(e) => setBusiness(e.target.value)} placeholder="Moss & Pine Cabin" style={{ width: '100%', boxSizing: 'border-box' }} />
                  </div>
                  <div className="form-group">
                    <label className="form-label" htmlFor="rs-city">City</label>
                    <input id="rs-city" className="form-input" type="text" value={city} onChange={(e) => setCity(e.target.value)} placeholder="Asheville" style={{ width: '100%', boxSizing: 'border-box' }} />
                  </div>
                  <div className="form-group">
                    <label className="form-label" htmlFor="rs-country">Home country <span style={{ color: 'var(--sage)', fontWeight: 400 }}>(optional)</span></label>
                    <input id="rs-country" className="form-input" type="text" value={country} onChange={(e) => setCountry(e.target.value)} placeholder="United States" style={{ width: '100%', boxSizing: 'border-box' }} />
                  </div>
                  <div className="form-group">
                    <label className="form-label" htmlFor="rs-website">Website <span style={{ color: 'var(--sage)', fontWeight: 400 }}>(optional)</span></label>
                    <input id="rs-website" className="form-input" type="text" value={website} onChange={(e) => setWebsite(e.target.value)} placeholder="https://yourproperty.com" style={{ width: '100%', boxSizing: 'border-box' }} />
                  </div>
                </>
              ) : (
                <>
                  <div className="form-group">
                    <label className="form-label" htmlFor="rs-instagram">Instagram handle</label>
                    <input id="rs-instagram" className="form-input" type="text" value={instagram} onChange={(e) => setInstagram(e.target.value)} placeholder="@yourhandle" style={{ width: '100%', boxSizing: 'border-box' }} />
                  </div>
                  <div className="form-group">
                    <label className="form-label" htmlFor="rs-tiktok">TikTok handle <span style={{ color: 'var(--sage)', fontWeight: 400 }}>(optional)</span></label>
                    <input id="rs-tiktok" className="form-input" type="text" value={tiktok} onChange={(e) => setTiktok(e.target.value)} placeholder="@yourhandle" style={{ width: '100%', boxSizing: 'border-box' }} />
                  </div>
                  <div className="form-group">
                    <label className="form-label" htmlFor="rs-portfolio">Portfolio or website <span style={{ color: 'var(--sage)', fontWeight: 400 }}>(optional)</span></label>
                    <input id="rs-portfolio" className="form-input" type="text" value={portfolio} onChange={(e) => setPortfolio(e.target.value)} placeholder="https://yourportfolio.com" style={{ width: '100%', boxSizing: 'border-box' }} />
                  </div>
                </>
              )}
              ...
              <button type="submit" className="btn-primary" disabled={submitting} style={{ width: '100%', cursor: submitting ? 'default' : 'pointer' }}>
                {submitting ? 'Submitting…' : `Sign up as ${roleLabel} →`}
              </button>
          ...
          <div style={{ textAlign: 'center', padding: '0.5rem 0 0.5rem' }}>
            ...
            <p style={{ color: 'var(--slate)', fontSize: '0.875rem', lineHeight: 1.55, margin: '0 0 1.5rem' }}>
              The Collabnb team will review your {roleLabel.toLowerCase()} details — the same check every new {roleLabel.toLowerCase()} goes through. Your account stays exactly as it is until you're approved.
            </p>
            <p style={{ fontSize: '0.78rem', color: 'var(--sage)', margin: '0 0 1.25rem' }}>
              We'll email you at <strong style={{ color: 'var(--ink)' }}>{useOtherEmail ? contactEmail : profile?.email}</strong> — usually within 1–2 business days.
            </p>
